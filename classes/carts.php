...
class Cart
{
                ...
                foreach ($_SESSION['cart'] as $product) {
                    ...
                    <ul>
                        <li class="d-flex justify-content-between mt-2">
                            <span class="text-muted" name="productName">' . $productName . '</span>
    ...
    {
        $productName = mysqli_real_escape_string($this->db->link, $product[1]);
        ...
        $query = "INSERT INTO `order_details`(`id`, `order_id`, `product_id`, `product_name`, `price`, `qty`, `total`, `created_at`, `updated_at`) 
        VALUES (0, $idUserCart, $productId, '$productName', $price, $quantity, $total, now(), now())";
        $result = $this->db->insert($query);
    }
    ...
    {
        $sql = "select *, products.name as pname, order_details.price as oprice, order_details.total as ototal from products, order_details where products.id=order_details.product_id and order_id=$id";
        $result = $this->db->select($sql);
        return $result;
    }

    public function showOrderDetailByUserId($userId)
    {
        // Chuẩn bị truy vấn SQL
        $sql = "SELECT od.*, o.fullname, o.phone, o.email, o.address, o.district, o.city, o.status, o.created_at AS order_created_at
    ...
    public function showOrderFilter($orderStatus = '', $fromDate = '', $toDate = '', $deliveryLocation = '')
    {
        ...
        if ($fromDate != '' && $toDate != '') {
            $conditions[] = "created_at BETWEEN '$fromDate' AND '$toDate'";
        }
        if ($deliveryLocation != '') {
            $conditions[] = "delivery_location LIKE '%$deliveryLocation%'";
        }

        // Nếu có điều kiện lọc, thêm vào câu truy vấn SQL
        if (!empty($conditions)) {
